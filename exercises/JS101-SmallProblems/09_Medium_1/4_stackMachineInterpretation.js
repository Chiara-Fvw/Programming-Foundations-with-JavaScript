...
function minilangMINE(orders) {
  let stack = [];
  let register = 0;

  orders.split(' ').forEach(order => {
    if (Number.isInteger(Number(order))) register = Number(order);
    if (order === 'PUSH') stack.push(register);
    if (order === 'ADD') register += stack.pop();
    if (order === 'SUB') register -= stack.pop();
    if (order === 'MULT') register *= stack.pop();
    if (order === 'DIV') register = Math.round(register / stack.pop());
    if (order === 'MOD') register = register % stack.pop();
    if (order === 'POP') register = stack.pop();
    if (order === 'PRINT') console.log(register);
  })
