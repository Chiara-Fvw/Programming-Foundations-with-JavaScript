...
/* 
Starting with the string: let munstersDescription = "The Munsters are creepy and spooky.";
Return a new string that swaps the case of all of the letters: `tHE mUNSTERS ARE CREEPY AND SPOOKY.`
...
function factors(number) {
  ...
  do {
    ...
    divisor -= 1;
  } while (divisor > 0); //one solution could be to change !== into >
  return factors;
}
...
function factorsBook(number) {
  ...
  while (divisor > 0) {
    if (number % divisor === 0) {
      factors.push(number / divisor); // if I just put push(divisor) the order of factors se invierte.
    }
    divisor -= 1;
  }
  return factors;
...
//concat merge two arrays and is a non distructive method. 
//push modify the array. 

/* Book: Yes, there is a difference. While both methods have the same return value, the first implementation mutates the argument represented by buffer. That is, the caller will see that the array is different when the function returns. The rollingBuffer2 implementation doesn't mutate the argument specified by the value of buffer.
 */
...
/* Chiara: Yes, because function pass by reference non primitive arguments 
  Book: Spot will find himself in the "dog house" for this one. The family's data is in shambles now.
...
//yes ERROR! respusta es NO el parametro default se usa solo si el argumento está vacío.
